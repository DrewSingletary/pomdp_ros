...
double segway_filter(void) {
  ...
  double maxVal = 0;
  for (int i = 0; i < totalGrids; i++) {
    double val = segway_belief_new[i] * obs_belief[i];
    maxVal += val;
  }
  if (maxVal > max_failure_rate) {
    ROS_INFO("FILTER ENGAGED FOR ACTION %i, h(x) = %f",segway_action,maxVal);
  }
  return maxVal;
}
...
void segway_actions_cb(const std_msgs::Int32::ConstPtr& msg) {
  ...
  if (prob_failure < max_failure_rate) {
    segway_action_real = segway_action;
  }
  else if (prob_failure > max_failure_rate && segway_action == 2) {
    segway_action = 3;
  ...
  else if (prob_failure > max_failure_rate && segway_action == 3) {
    segway_action = 2;
    prob_failure = segway_filter();
    if (prob_failure > max_failure_rate) {
      segway_action_real = 0;
